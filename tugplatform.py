...
def make_plotly_fig(t: np.ndarray, norm: np.ndarray, event_times: Dict[str, float], title: str = "") -> go.Figure:
    ...
    for key, label in EVENTS:
        ...
        if v is None:
            continue
        fig.add_vline(x=float(v), line_dash="dash")
        fig.add_annotation(x=float(v), y=y_max, text=label, showarrow=False, textangle=-90, yanchor="top")
...
with tabs[3]:
    ...
    train_file = st.file_uploader("Arquivo de treino (txt/csv com ';')", type=["txt", "csv"], accept_multiple_files=False)
...
with tabs[4]:
    ...
    st.caption("Pré-processamento aplicado: detrend + interpolação 100 Hz + lowpass 1.5 Hz; visualização da norma.")
